...
| ------------ | -------------------------------------- | ----------------------------------------------------------------------------- |
| Primitives   | Button, Input, Label, Avatar, TextArea, Microcopy | `asChild` pattern via Radix `Slot` everywhere. Button supports icon children. TextArea and Microcopy added 2026-08-15 — not in the original spec, see dated notes below. Badge removed from this row 2026-08-15 — it's a locator-app-local component (`apps/locator`), not a shared `packages/ds` export; see the Phase 2 sections below, which still reference it in that app-local sense. |
