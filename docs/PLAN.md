...
- **Fix: swap `Root` to a single-column CSS grid, `grid-cols-[minmax(max-content,1fr)]`, plus an explicit `w-fit` on `Root` itself.** The grid column alone handles the *shared-width* half of the fix — `Label`/`Group`/`Microcopy` now sit in one column whose minimum width is `max-content` across all three, with `justify-items: stretch` (grid's default) filling each row to that shared width, so `Group` correctly stretches to match a long `Label` instead of just its own items (added per user request as a distinct default-sizing requirement, not just the stretch-bug fix). But `display: grid` alone doesn't shrink-wrap a block-level container's own *outer* box — without `w-fit`, `Root` still filled 100% of its parent's width regardless of the column definition, since a block box's `width: auto` means "fill available inline space," not "shrink to content." Caught only by checking the live Storybook render, not by reasoning about the grid spec in the abstract — the first attempt (grid column alone, no `w-fit`) looked identical to the original bug when actually tested.
