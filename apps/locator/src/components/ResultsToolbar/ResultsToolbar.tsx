...
// simultaneously (`hidden`/`min-[920px]:hidden` toggling which is
// visible), same dual-mount convention AdvisorSearchModule's Start/
// InProgress stages and FilterFacets already use for a device-class (not
// component-width) layout decision, rather than a container query.
//
// The switch itself is 920px, not `md` (768px) -- per the user, matching
// Results.tsx's own `min-[920px]:flex-row` Dual-view side-by-side
// breakpoint: Dual only ever becomes selectable once the page has
// actually laid it out side by side, not while it'd still render
// List/Map stacked full-width underneath each other.
//
...
export function ResultsToolbar({
  ...
  return (
    <div className={cn(className)}>
      ...
      <div className="hidden min-[920px]:flex min-[920px]:items-end min-[920px]:justify-between">
      ...
      {/* Mobile: `Results (N)` heading and the radio-button pair share one
          ...
          this container and the viewport edge below 920px (this block's
          own visibility threshold, below), so the offset is constant and
          this threshold reproduces the requested viewport breakpoint
          exactly. A 3-column `1fr auto 1fr` grid (not flex) keeps the
          radio-button pair centered in the row's full width independent
          of the heading's text length, per the user -- the empty 3rd
          column balances the 1st so the auto-sized 2nd column lands dead
          center; below the threshold it collapses back to one column, so
          `justify-self-start`/`-center` on the two children reproduce
          the same stacked, centered-pair look as before.
          `view === 'dual'` can only be selected at 920px+, but stays
          possible in state if the viewport narrows afterward -- falls
          back to `list` here so a radio option is always shown as
      ...
      <div className="@container min-[920px]:hidden">
